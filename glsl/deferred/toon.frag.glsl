...
void main() {
    ...
    // Calculate the specular and diffuse coefficient terms
    float diffuse = diffuseLighting(nor, lightDir);
    float specular = specularLighting(nor, lightDir, viewDir);

    // Toon shading effects.
    float edge = (dot(viewDir, nor) > 0.3) ? 1.0 : 0.0;
    // if (dot(viewDir, nor) > 0.3) {
    //     edge = 1.0;
    // }

    // Ramp on diffuse term. Use hard threshold for specular term.
    float spec = (specular > 0.1) ? 1.0 : 0.0;
    diffuse = floor(diffuse * levels) * scale;

    vec3 color = edge * colmap * u_lightCol * (diffuse + specular * spec) * attenuation;
