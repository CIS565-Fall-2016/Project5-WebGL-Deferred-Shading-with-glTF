...
void main() {
    ...
    // Cel shading
    float l_dist = texture2D(u_depth, v_uv - .004).x - depth;
    float r_dist = texture2D(u_depth, v_uv + .004).x - depth;
    float eps = .01;
    if (l_dist > eps || r_dist > eps) {
    ...
    if (dist >= u_lightRad) {
        ...
        gl_FragColor = vec4(0, 0, 0, 1);// (1. - depth) * .1 * vec4(colmap, 1);
        return;
    }
    ...
    lambertian *= 2.;
    lambertian = floor(lambertian);
    lambertian /= 2.;

    float attenuation = max(0.0, u_lightRad - dist);
    attenuation /= u_lightRad;  
    gl_FragColor = lambertian * vec4(colmap, 1)  * vec4(u_lightCol, 1.0);
}
